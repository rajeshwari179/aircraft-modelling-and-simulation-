...
for sheet_name in pd.ExcelFile(excel_file).sheet_names:
    ...
    df.drop(df[df['AoA'] != AoA].index, inplace = True)
    df = df.sort_values(by=['Altitude','Mach'])
    ...
    for i in range(5,6):
        col = i + 1
        for j in range(0,num_Altitude):
            chart.add_series({
                'name':       [sheet_name, 1+j*num_Mach, 0],
                'categories': [sheet_name, 1, 2, max_row, 2],
                'values':     [sheet_name, 1+num_Mach*j, col, num_Mach*j+num_Mach, col],
                'marker':     {'type': 'circle', 'size': 7},
            })
    
    # Configure the chart axes.
    chart.set_x_axis({'name': 'Mach [-]'})
